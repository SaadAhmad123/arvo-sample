...
export const usePromise = <T extends unknown[], R>(
  ...
  const state = useReactiveState<UsePromiseState>(UsePromiseState.idle);
  ...
  const reset = useCallback(() => {
    state.set(UsePromiseState.idle);
    setError(undefined);
    setData(undefined);
  ...
  const retry = useCallback(
    async (...arg: T) => {
      state.set(UsePromiseState.loading);
      setError(undefined);
      let resp = undefined as R | undefined;
      try {
        resp = await retryPromise(promiseFn, retryCount, retryDelay)(...arg);
        state.set(UsePromiseState.success);
        setData(resp);
      } catch (e) {
        setError(e as Error);
        state.set(UsePromiseState.error);
        if (throwError) {
          throw e;
        }
      }
