...
export const reflectorAgentMachineV100 = setupArvoMachine({
  ...
  actions: {
    ...
    setCriticResponse: assign(({ context, event }) => {
      if (
        ...
        if (!rawGeneration) return context;
        const generationCritique = parsedResponse.data.analysis.map((item) => ({
          criterion: context.configuration.criteria[item.id] ?? '',
          satisfied: item.satisfied,
          improvement: item.improvement,
        }));
        const evaluationScore = generationCritique.length
          ? generationCritique.filter((item) => item.satisfied).length / generationCritique.length
          : 1;
        ...
        return {
          generations: [...context.generations, generation],
        };
      }
      return context;
    }),
  ...
  output: ({ context }) => {
    try {
      ...
      const bestGeneration = context.generations.reduce(
        (acc, cur) => (cur.evaluation_score >= acc.evaluation_score ? cur : acc),
      ...
      return {
        ...
        result: {
          total_token_usage: context.generations.reduce(
            (acc, cur) => acc + cur.total_token_usage.critique + cur.total_token_usage.generation,
            0,
          ),
