...
import { SQSEvent, SQSRecord, EventBridgeEvent, APIGatewayProxyResult } from 'aws-lambda';
...
export const handler = async (event: LambdaEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    const isEventBridgeEvent = (event: LambdaEvent): event is EventBridgeEvent<string, CustomEventDetail> => {
    ...
    const handler = Services.anthropicCompletions({
      settings: async () => ({
        ANTHROPIC_API_KEY: '',
      }),
    });
...
handler({
  ...
  subject:
    'eJwljdEOwiAMRf+lz0JgQ5n+gZ9RaY0kDkzpjMmyf5fpW2/vzTkrVEkPbiqoVeCyQsGZ4QKpzhaLPqS+crI9vZ6suZYGB3iztH72lbfOOtgOwB9Oi/6eK2Tq1TSS85GiOSOeTODRG5z8YNifcbg5uuHdd1YuWfPfDY3lnRPbKpmLMlmpi7Ls/JkVd3Tjtquvu8FRGEOMdxMjkgk0oemCwcTgaBySx2PwsG1fcfhIGg==',
  data: {
    max_tokens: 4096,
